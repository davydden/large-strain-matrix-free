...
\begin{algorithm}[!ht]
  ...
  \ForEach{ element $K \in \Omega^h$ }{
        ...
        $\Grad \, \overline{\gz u^h}_K$ \tcp*{2nd order}
        $\gz g_s := \overline{\grad^s} \gz x_K$ \tcp*{2nd order symmetric}
        $\gz g := \overline{\grad} \gz x_K$ \tcp*{2nd order}
...
{\color{red}
The main reason is that the operations are typically the same on all elements.
For a given quadrature point, we simultaneously perform tensor evaluations and contractions on all elements\footnote{For the heterogeneous materials considered here,
...
This functionality is provided by the \texttt{deal.II} library within the \textit{MatrixFree} class.
}.
...
For more information about the implementation details and data structures for sum factorization approaches in deal.II (including hanging constraints, storage of the inverse Jacobian of the transformation from unit to real cell, MPI and shared memory parallelization as well SIMD vectorization) and performance tests, see \cite{kronbichler12}.
}
...
Finally, we note that at the time of writing, $p-$ version of the GMG is not available within the \texttt{deal.II} library and therefore not considered here.
}
...
Note that for the matrix-free approach we are limited to smoothers that use only diagonal elements of the operator.
}
...
On another hand, the problem size is large enough so that the sparse matrix will not fit into the L3 cache and therefore we can observe that the matrix-based approach is memory bound.
}
%
...
The performance results for finite-strain elasticity operators considered in this study are qualitatively similar to those reported for Laplace operator in \cite{kronbichler12}.
...
Note that for the Algorithm \ref{alg:mf_scalar} we need to evaluate gradients with respect to both deform and undeformed
configurations, and therefore two inverses will be cached via two different \textit{MatrixFree} objects.\todo{rerun RRZE and IWR}\todo{this is roughly main memory transfer, do we elaborate on this/use it?}}
...
\begin{figure}[!ht]
  ...
  \begin{subfigure}[b]{0.8\textwidth}
    \centering
    \includegraphics[width=\textwidth]{LIKWID_Emmy_RRZE_roofline.pdf}
    % \caption{2D multigrid mesh with 2 global refinements}
  \end{subfigure}
  \caption{Roofline model for a selection of polynomial degrees on a 2D test example.}%
  \label{fig:roofline}
\end{figure}

{\color{red}
Figure \ref{fig:roofline} shows the Roofline performance model.
As expected, the matrix-based approach is memory bound and (on average) achieves the performance of $8.5\,\rm{GFlop/s}$ (4.5\% of the peak performance).
Matrix-free implementation of fine-strain elastic tangent operators (on average) leads to a much better performance of
$22.6\,\rm{GFlop/s}$, as well as about an order of magnitude higher computational intensity, but still stays in the memory bound regime.
The achieved performance is about 13\% of the peak arithmetic performance, which is considerably lower than the
70\% reported for the Laplace operation on Cartesian meshes in \cite{kronbichler12}.
...
we observe very marginal increase in computational intensity and almost unchanged performance.
This indicates that the origin for suboptimal results of matrix-free approach is most likely not related to the algorithms numerically investigated here.
Further detailed studies are necessary to explain this behavior.
}

{\color{red}
To summarize, we can attribute the speedup of the matrix-free finite-strain tangent operators for higher order bases to two factors.
...
{\color{red}
Roofline model indicated the matrix-free finite-strain elasticity tangent operators have an order magnitude higher algorithmic intensity. Further studies are required to identify means to improve the computational intensity and performance of the matrix-free implementation.}
