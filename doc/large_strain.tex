...
The location of a material particle $\gz X$ is given by $\gz x = \gz \varphi_t (\gz X)$.
...
where $\gz F := \Grad \, \gz \varphi \equiv \gz I + \Grad \, \gz u$ is called deformation gradient.
The mapping has to be one-to-one and must exclude self-penetration. Consequently, the Jacobian has to be positive $J = \det \gz F > 0$. The deformation gradient can be split into the proper orthogonal rotation tensor $\gz R$ and the symmetric right $\gz U$ and left $\gz V$ stretch tensors such that $\gz F = \gz R \gz U = \gz V \gz R$.
...
% strain measures
%
In what follows, we characterize the material behavior using the Green-Lagrange strain tensor $\gz E:=\frac{1}{2}\left[\gz C - \gz I\right]$, where $\gz C$ is the right Cauchy-Green tensor $\gz C := \gz F^T \cdot \gz F = \gz U^2$. Clearly $J = \sqrt{\det \gz C}$ and $\partial (\bullet)/\partial \gz E = 2\, \partial (\bullet)/\partial \gz C$. We will also use the left Cauchy-Green tensor $\gz b := \gz F \cdot \gz F^T = \gz V^2$.

For conservative systems without volumetric load, the total potential energy functional $\mcl E$ is introduced as
\begin{align}
\mcl E =
...
In addition to the standard mapping from the reference (unit / isoparametric) element to the element in real space, this class computes the mapping to the deformed configuration given a displacement field.
%
Due to the specifics of matrix-free operator evaluation implemented in \texttt{deal.II} the integration is done over the deformed configuration. Therefore, we have to divide by the Jacobian $J$ of the mapping at a given quadrature point additionally to express the integral in the undeformed configuration.
...
In order to avoid recalculation of the Kirchhoff stress, Algorithm \ref{alg:mf_tensor2} caches its value for each element and quadrature point. In this case, we also avoid the need to evaluate gradients of displacement field with respect to the undeformed configuration. This algorithm utilizes the chosen constitutive relationship. Therefore, the action of the material fourth order tangent $J \boldsymbol{\mathcal{C}}$ on the second order symmetric tensor $\gz g_s$ is cheap to evaluate, see \eqref{eq:simplified_action}. To that end, we cache two scalars that depend on the Jacobian $J$.
...
In this case, we have to cache the fourth order symmetric tensor $\boldsymbol{\mathcal{C}}$ for each element and quadrature point and perform the double contraction with the second-order symmetric tensor on-the-fly.
...
The efficiency of standard iterative solvers deteriorate for large systems of linear equations due to incresing iteration counts under mesh refinement.
On the other hand, geometric multigrid (GMG) preconditioners \cite{Bramble1990, Briggs2000, Janssen2011,May2015} can be proven to result in iteration counts independent of the number of mesh refinements by smoothing the defect on a hierarchy of levels. For further information about geometric multigrid methods we refer the reader to \cite{Briggs2000,Hackbusch1985,Wesseling1992}.
