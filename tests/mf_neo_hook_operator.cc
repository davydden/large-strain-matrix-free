...
public:
  ...
  double
  value(const Point<3> &p, const unsigned int component) const
  {
    const Tensor<2, 3> matrix =
      Physics::Transformations::Rotations::rotation_matrix_3d(
        Point<3>(0, 0, 1.), 0.6 * p[2]);
    Tensor<1, 3> t   = p;
...
void
test_elasticity(const Function<dim> &displacement_function)
{
  ...
  displacement.reinit(owned_set, relevant_set, MPI_COMM_WORLD);

  {
    VectorTools::interpolate(dof, displacement_function, displacement);
  ...
  auto mapping = std::make_shared<
    MappingQEulerian<dim, LinearAlgebra::distributed::Vector<number>>>(
    fe_degree, dof, displacement);
  // auto mapping =
  // std::make_shared<MappingFEField<dim,dim,LinearAlgebra::distributed::Vector<number>>>(dof,displacement);
  ...
  std::shared_ptr<MatrixFree<dim, number>> mf_data_current(
    new MatrixFree<dim, number>());
  std::shared_ptr<MatrixFree<dim, number>> mf_data_reference(
    new MatrixFree<dim, number>());
  ...
  Material_Compressible_Neo_Hook_One_Field<dim, VectorizedArray<number>>
                                                        material(mu, nu, material_formulation);
  Material_Compressible_Neo_Hook_One_Field<dim, number> material_standard(
    mu, nu, material_formulation);

  // before going into the cell loop, for Eulerian part one should reinitialize
  // MatrixFree with initialize_indices=false as the mapping has to be
  // recomputed but the topology of cells is the same
...

  {
    ...
    fe_values_ref[u_fe].get_function_gradients(displacement,
                                               solution_grads_u_total);
    dof_cell->get_dof_indices(local_dof_indices);
    dof_cell->get_dof_values(src, src_local.begin(), src_local.end());
    ...
    for (unsigned int q = 0; q < n_q_points; ++q)
      {
        ...
        const Tensor<2, dim, VectorizedArray<number>> F_bar =
        ...
        const SymmetricTensor<2, dim, VectorizedArray<number>> jc_part =
          material.act_Jc(det_F, b_bar, b, symm_grad_Nx_v);
        ...
        //=================
        // Grad u
        const Tensor<2, dim, number> &grad_u_standard =
          solution_grads_u_total[q];
        const Tensor<2, dim, number> F_standard =
          Physics::Elasticity::Kinematics::F(grad_u_standard);
        const SymmetricTensor<2, dim, number> b_standard =
          Physics::Elasticity::Kinematics::b(F_standard);
        const number                 det_F_standard = determinant(F_standard);
        const Tensor<2, dim, number> F_bar_standard =
          Physics::Elasticity::Kinematics::F_iso(F_standard);
        const SymmetricTensor<2, dim, number> b_bar_standard =
          Physics::Elasticity::Kinematics::b(F_bar_standard);
        ...
        for (unsigned int k = 0; k < dofs_per_cell; ++k)
          {
            ...
            symm_grad_Nx_v_standard +=
              src(local_dof_indices[k]) * symm_grad_Nx[k];
          }

        SymmetricTensor<2, dim, number> tau_standard;
        material_standard.get_tau(tau_standard,
                                  det_F_standard,
                                  b_bar_standard,
                                  b_standard);
        ...
        const SymmetricTensor<2, dim, number> jc_part_standard =
          material_standard.act_Jc(det_F_standard,
                                   b_bar_standard,
                                   b_standard,
                                   symm_grad_Nx_v_standard);
        const Tensor<2, dim> geo_standard_v =
          egeo_grad(grad_Nx_v_standard, tau_ns_standard);

        for (unsigned int i = 0; i < dofs_per_cell; ++i)
          for (unsigned int j = 0; j <= i; ++j)
            {
              cell_matrix(i, j) +=
                (symm_grad_Nx[i] * material_standard.act_Jc(det_F_standard,
                                                            b_bar_standard,
                                                            b_standard,
                                                            symm_grad_Nx[j])) *
                JxW;
              const Tensor<2, dim> geo_standard =
                egeo_grad(grad_Nx[j], tau_ns_standard);
              cell_matrix(i, j) +=
                double_contract<0, 0, 1, 1>(grad_Nx[i], geo_standard) * JxW;
            }
          ...
          for (unsigned int j = 0; j < dim; ++j)
            {
              ...
              grad_Nx_v_diff[i][j] =
                grad_Nx_v_standard[i][j] - grad_Nx_v[i][j][0];
        ...
        AssertThrow(jc_diff.norm() < 1e-12 * jc_part_standard.norm(),
                    ExcMessage("Jc"));
        AssertThrow(geo_diff.norm() < 1e-12 * geo_standard_v.norm(),
                    ExcMessage("geo"));
    ...
    for (unsigned int i = 0; i < dofs_per_cell; ++i)
      {
        ...
        std::cout << diff << " ";
        AssertThrow(std::abs(diff) < 1e-10 * std::abs(dst_local[i]),
                    ExcInternalError());
      }
    std::cout << std::endl;
...
main(int argc, char **argv)
{
  ...
  std::cout << "=====================" << std::endl
            << "UniaxialTension" << std::endl;
  test_elasticity<2, 1>(UniaxialTension<2>());
  std::cout << "=====================" << std::endl
            << "UniaxialTension2" << std::endl;
