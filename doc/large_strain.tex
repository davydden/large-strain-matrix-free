...
\label{eq:tangent}
\end{equation}
Here $\overline{(\bullet)}$ is used to denote quantities evaluated using the displacement field $\overline {\gz u}$, and the fourth-order {\color{red}spatial} tangent {\color{red}stiffness} tensor is the push forward of the material {\color{red}part of the referential} tangent {\color{red}stiffness tensor} $J \boldsymbol{\mathcal{C}} = \chi\left( 4 \frac{d^{2} \psi \left( \mathbf{C} \right)}{d \mathbf{C} \otimes d \mathbf{C}} \right)$.
The first term in \eqref{eq:tangent} is related to the linearization of the Piola-Kirchhoff stress $\gz S$ and {\color{red}is} therefore called material part of the tangent.
The second term in \eqref{eq:tangent} is called {\color{red}the} geometric part of the tangent since it originates from the linearization of $\gz F$, $\gz F^T$ and $\gz F^{-1}$.
...
\begin{algorithm}[!ht]
  ...
  \caption{Matrix-free tangent operator: cache fourth-order material tangent {\color{red}stiffness tensor} $\boldsymbol{\mathcal{C}}$ and Kirchhoff stress $\gz \tau$.}
