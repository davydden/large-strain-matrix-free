...
\pgfkeys{/pgf/number format/fixed,/pgf/number format/precision=3}
...
In practise this implies that parallelization of the assembly operation (for the matrix-based approach) and the linear solver (as well as various pre- and post-processing steps), both of which are the focal points of investigation in this manuscript, has been achieved using MPI.
...
configurations, and therefore two inverses will be cached via two different \textit{MatrixFree} objects.\todo{this is roughly main memory transfer, do we elaborate on this/use it?}}
...
{\color{red} demonstrates the same \changeJP{trend in terms of} the number of CG iterations, however it}
