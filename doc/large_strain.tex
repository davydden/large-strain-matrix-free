...
\newcommand{\changeJP}[1]{\textcolor{blue}{#1}}
\newcommand{\todojp}[1]{\todo[inline,color=red!40,size=\small]{TODO (J-P): #1}}
...
This can potentially reduce the advantage of matrix-free methods, that are generally more competitive for higher order elements\todo{reference? Martin's papers?}.
...
and performance {\color{red} on a single node of a high performance computing cluster.}
\changeJP{
...
In \ref{sec:mf} we describe the different matrix-free operator evaluation approaches that are evaluated in this study, and in section \ref{sec:gmg} we propose a geometric multigrid preconditioner that is suitable for finite-strain hyperelasticity.
}
...
\changeJP{Lastly, the} results are summarized in Section \ref{sec:summary}.
...
\section{\changeJP{Description of numerical framework}
...
This library offers a number of paradigms by which to parallelize a finite-element code.
...
For the matrix-based approach, the Trilinos \cite{Heroux2005} library, version 12.12.1 was leveraged; 
the system tangent matrix is stored in a distributed \textit{Epetra\_FECrsMatrix} from the Epetra package \cite{Heroux2005b} sparse data structure and the linear solver was preconditioned with an algebraic multigrid (AMG) preconditioner from the package ML package \cite{Gee2006a}.
...
As will be detailed later, the pre-existing matrix-free framework that was used offers further opportunity for low-level parallelism that may be exploited.
...
{\color{red}For the performance of the matrix-free operator, it is crucial to find a good intermediate point between precomputing everything (caching) and evaluating all the quantities on-the-fly.}
%
...
\begin{algorithm}[!ht]
  ...
  \ForEach{ element $K \in \Omega^h$ }{
        {\color{red}extract local vector values on this element $\overline{\gz u^h}_K$, $\gz x_K$ } \;
...
\begin{algorithm}[!ht]
  ...
  \Output{action of the FE tangent operator \eqref{eq:algebraic_tangent} on $\gz x$}
  \ForEach{ element $K \in \Omega^h$ }{
        {\color{red}extract local vector values on this element $\gz x_K$ } \;
        evaluate the following tensors at each quadrature point using sum factorization :\\
        \enskip$\gz g_s := \overline{\grad^s} \gz x_K$ \tcp*{2nd order symmetric}
        \enskip$\gz g := \overline{\grad} \gz x_K$ \tcp*{2nd order}
        \ForEach{quadrature point $q$ on $K$}{
...
\changeJP{In general, not all quadrature points in each element group are endowed with identical, non-dissipative constitutive laws that follow the same code path during evaluation or the kinetic quantities and tangents.
...
For example, {\color{red}the} deformation gradient $\gz F = \gz I + \Grad \, \overline{\gz u^h}$ (second-order tensor) is evaluated at the specific quadrature point of all elements within the ``block''.
...
The efficiency of the preconditioner with the considered level operators will be assessed by numerical examples in the next section.
...
\changeJP{%
Aligned with the approach taken in similar previous investigations \cite{}\todo{Cite Martin's papers?}, in this study we limit ourselves to comparisons made on a node level.
...
%
% Below are are implementational details that might affect the comparative result
%
...
The results of the matrix-free approach with a geometric multigrid preconditioner are compared to the matrix-based approach in conjunction with the algebraic multigrid (AMG) preconditioner using the package ML \cite{Gee2006a} of the Trilinos \cite{Heroux2005} library.
...
{\color{red}Unless noted otherwise, the simulations are performed with 20 MPI processes, i.e. \changeJP{one} fully occupied node.}
...
configurations, and therefore two inverses will be cached via two different \textit{MatrixFree} objects.\todo{this is roughly main memory transfer, do we elaborate on this/use it? I don't think so}}
...
Turbo mode of CPU was disabled.
}
...
\todo{Discuss MPI/SIMD}
\input{parallelization_2d.tex}
\input{parallelization_3d.tex}
...
% Further optimization of \texttt{Tensor} \changeJP{and \texttt{SymmetricTensor}}classes in \textit{deal.II} are required to
...
{\color{red} demonstrates the same \changeJP{trend in terms of} the number of CG iterations, but}
...
Our goal is to \changeJP{determine} whether the matrix-free approach with GMG preconditioner can be faster than the matrix-based approach with an AMG preconditioner.
}
\changeJP{We note that although the well-established implementation of the AMG preconditioner \cite{Gee2006a} is the best available choice for the matrix-based implementation of the boundary value problem, the current implementation of the matrix-free GMG preconditioner is not yet optimized and can be improved further.}
...
{\color{red}Most importantly, the wall time per DoF for the matrix-free solver with GMG preconditioner stays almost constant both for 2D and 3D problems.
...
{\color{red}Among the three \changeJP{examined} algorithms,}
the implementation that caches the material part of the fourth-order spatial tangent stiffness tensor together with the second-order Kirchhoff stress was shown to be faster than the matrix-based approach for polynomial degrees higher than one in 3D and polynomial degrees higher than two {\color{red} in 2D}.
{\color{red}
...
% Further studies are required to identify means to improve the computational intensity and performance of the matrix-free framework provided by \texttt{deal.II}.}
...
% as opposed to node level considered here.
}
...
D.~Davydov, D.~Arndt and J-P.~Pelteret are grateful to {\color{red}Martin Kronbichler (TU Munich),} Jed Brown (CU Boulder) and Veselin Dobrev (Lawrence Livermore National Laboratory) for \changeJP{fruitful discussions} on matrix-free operator evaluation approaches.
...
\todo{Add back of the envelope calculations by Veselin Dobrev? See {https://github.com/CEED/Forum/issues/1}.}
