...
a vector space spanned by standard vector--valued FE basis functions $\gz N_i(\gz x)$ (e.g. polynomials with local support):
\begin{alignat}{2}
...
\begin{align}
  K_{ij} \Delta u_j &= - F_i \\
  K_{ij} &=
  \int_{\mcl B_0} \overline{\grad^s} \gz N_i : J \boldsymbol{\mathcal{C}} : \overline{\grad^s} \, \gz N_j \d V
  +
  \int_{\mcl B_0}
...
\end{align}
Here $\gz K$ is the discrete tangent operator and $\gz F$ is the discrete gradient.
