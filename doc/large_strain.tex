...
the linear solver is preconditioned with an algebraic multigrid (AMG) preconditioner from the ML package \cite{Gee2006a}.
...
We choose a problem size on the order of $10^6$ DoFs for each test case as this is an upper limit due to the memory requirements of the matrix-based approach with a high polynomial degree.
...
{\color{red}
Algorithm \ref{alg:mf_tensor2} is recommended when linearization of the adopted material model allows to efficiently implement the action of the material part of the fourth-order spatial tangent stiffness tensor {\color{red}by a second-order symmetric tensor}.
...
The GMG has been applied to a heterogeneous material assuming that the coarsest level can provide an adequate discretization of the heterogeneity.
