...
$22.6\,\rm{GFlop/s}$ in 2D, as well as about an order of magnitude higher computational intensity, but still stays in the memory bound regime.
...
Unfortunately, the performance measurements for non-Cartesian meshes were not reported in \cite{kronbichler12}.
...
As a result both algorithms will have comparable memory requirements (also confirmed by Figure ....),
...
Finally we note that all three algorithms are far from the theoretical peak performance which
indicates that the origin for suboptimal results of matrix-free approach is most likely not related to the various flavours of algorithms numerically investigated here.
Further detailed studies are necessary to explain this behavior.

To summarize this section, we can attribute the speedup of the matrix-free finite-strain tangent operators for higher order bases to two factors.
