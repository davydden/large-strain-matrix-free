...
either in referential or current configurations, at the quadrature points that is cheap enough to be evaluated on the fly.
}
...
The deformation of a body $\mcl B$ from the referential configuration $\mcl B_0$ to the {\color{red}current} configuration $\mcl B_t$ at time $t$
...
Note that $\overline{\grad^s} \gz N_i$ and $\overline{\grad} \gz N_i$ are gradients of shape functions in the {\color{red}current} configuration,
...
%
In order to obtain the gradients with respect to the {\color{red}current} configuration,
...
to the {\color{red}current} configuration given a displacement field.
%
Due to the specifics of matrix-free operator evaluation implemented in \texttt{deal.II}, the integration is eventually performed over the {\color{red}current} configuration.
...
current configurations, respectively. Algorithm~\ref{alg:mf_scalar} involves
...
Given that in this case we need to evaluate gradients with respect to both referential and current configurations as listed in Table~\ref{tab:memory},
...
{\color{red}the implementation that caches the second-order Kirchhoff stress tensor and evaluates terms in the current configuration is faster than
