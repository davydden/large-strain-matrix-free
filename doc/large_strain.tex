...
{\color{red} which is also limited to small strain linear elasticity.omit?}
...
This can potentially reduce the advantage of matrix-free methods, that are generally more competitive for higher order elements ({\color{red} reference?}).
...
On the coarsest level we use a CG solver to reduce the residual by three orders of magnitude within (at most) one hundred steps.
Since this smoother only uses diagonal elements of the operator it is readily compatible with the matrix-free approach while must default smoothers rely on an explicit  matrix form.}
...
\begin{figure}[!ht]
  ...
  `w/o FMA' represents a ceiling without Fused Multiply-Add ($P/2$), finally `w/o SIMD' is a ceiling where both FMA
    and SIMD vectorization are discarded ($P/8$).{\color{red} TODO Improve design}}%
  \label{fig:roofline}
\end{figure}
