...
where the matrix-vector products are formed on-the-fly \cite{kronbichler12,May2015, Krank2017, Brown2010, Gmeiner2016},
...
We are only aware of a single paper in preparation that deals with small strain linear elasticity \cite{Clevenger2018} and a masters thesis produced by a student of two of the authors \cite{Mentler2017}.
...
Finite element method (FEM) with linear or quadratic elements (that reduce the potential for locking), or mixed or enhanced formulations (that avoid locking) are often adopted in the solid mechanics community.
Even higher order elements are rarely employed.
However, we note, even with linear FEM, that large scale computations with $10^{12}$ unknowns inevitably requires a matrix-free approach as there is simply not enough memory to store the sparse tangent matrix \cite{Gmeiner2016}. Therefore, we believe that for large scale computations in solid mechanics it is crucial to consider matrix-free approaches and develop efficient multigrid solvers.

In this work, we investigate different implementations of the finite-strain hyperelastic tangent operator with respcet to their computational cost.
...
is defined via the mapping $\gz \varphi_t: \mcl B \rightarrow \mathbb E^3$, which places material points of $\mcl B$ in the Euclidean space $\mathbb E^3$.
...
%
We assume that the deformation map is linear such that
...
We shall denote the Piola transformation of rank-2 and rank-4 tensors $\mathbf{A}$ and $\boldsymbol{\mathcal{A}}$ by
\begin{align}
...
% strain measures
%
In what follows, we parameterize the material behavior using the right Cacuhy-Green tensor $\gz C := \gz F^T \cdot \gz F$.
...
%
For conservative systems without the body force, the total potential energy functional $\mcl E$ is introduced as
\begin{align}
\mcl E =
...
The principal of stationary potential energy requires for all  equilibrium states $\gz u$ that the directional derivative with respect to the displacement
\begin{align}
\delta \mcl E =
...
= 0 \, ,
\end{align}
where $\gz P := \partial \mcl \psi / \partial \gz F$ is the first Piola-Kirchhoff stress tensor.
...
(\gz \tau \cdot \gz F^{-T}) : \Grad \, \delta \gz u =
\gz \tau : (\Grad \, \delta \gz u \cdot \gz F^{-1}) =
...
Note that $\gz \tau$ is the Piola transformation of the second Piola-Kirchhoff stress $\gz S := \partial \mcl \psi / \partial \gz E \equiv 2 \partial \mcl \psi / \partial \gz C$,
...
\label{eq:tangent}
\end{align}
Here $\overline{(\bullet)}$ is used to denote quantities evaluated using the displacement field $\overline {\gz u}$, and the fourth order tangent tensor is the push forward of the material tangent $J \boldsymbol{\mathcal{C}} = \chi\left( 4 \frac{d^{2} \psi \left( \mathbf{C} \right)}{d \mathbf{C} \otimes d \mathbf{C}} \right)$. The first term in \eqref{eq:tangent} is related to the linearization of the Kirchhoff stress and therefore called material part of the tangent operator.  The second term in \eqref{eq:tangent} is called geometric part of the tangent since it originates from the linearization of the inverse of the deformation gradient $\gz F^{-1}$.
...
\end{align*}
where in the last step $\tilde{a}_K(\varphi_i,\varphi_j)(\gz x_q)$ is a representation for the evaluation of the bilinear form in one quadrature point $q$.
For the sake of demonstration, let us assume that $A$ represents a mass matrix. In this case, it holds that
\begin{align*}
...
and this is the same as for a regular matrix-vector product where the entries are already precomputed. Assuming tensor product ansatz spaces and a tensor product quadrature rule, evaluation of this operations can be performed more efficiently.
Indeed, the evaluation of all the shape functions in all the ansatz points can be rewritten as
\begin{lstlisting}
...
Hence, we can expect that such a matrix-free approach is faster than a regular matrix-based approach, especially for 3D and high polynomial degree with the consistent quadrature rule.
More information on techniques developed to increase the computational density can be found in \cite{kronbichler12,vos10}.
...
Below, we propose three algorithms to implement tangent operator of finite-strain linear elasticity (see \eqref{eq:algebraic_tangent}) using the matrix-free approach.
%
...
Algorithm \ref{alg:mf_scalar} caches a single scalar which involves the logarithm of the Jacobian -- a computationally demanding operation compared to additions and multiplications. As evident from Section \ref{sec:theory}, we need to reevaluate the Kirchhoff stress at each quadrature point in order to apply the operator. This in turn requires evaluating the deformation gradient $\gz F$ and therefore gradients with respect to the undeformed configuration.
...
This algorithm also utilizes the chosen constitutive relationship in that the operation of $J \boldsymbol{\mathcal{C}}$ remains directly expressed in the operator itself. It is, however, somewhat more generic than Algorithm \ref{alg:mf_tensor1} since the dependence of $\gz \tau$ exists outside of the oeperator itself (although it must remain consistent with the action of $J \boldsymbol {\mathcal{C}}$).
Therefore, the action of the material fourth order tangent $J \boldsymbol{\mathcal{C}}$ on the second order symmetric tensor $\gz g_s$, as defined in \eqref{eq:simplified_action}, is cheap to evaluate.
...
In this case, we have to cache the fourth order symmetric tensor $\boldsymbol{\mathcal{C}}$ and Kirchhoff stress $\gz \tau$ for each element and quadrature point, and perform the double contraction with the second-order symmetric tensor $\gz g_s$ on-the-fly.
...
Note that this differs from the classical approach where the (tangent) operator $A^{l+1}$ on level $l+1$ is directly related to the (tangent) operator $A^{l}$ on level $l$ via $A^{l+1}=I^{l+1}_{l}  A^l I^l_{l+1}$, where $I^l_{l+1}$ and $I^{l+1}_l$ are global prolongation (coarse-to-fine) and restriction (fine-to-coarse) operators, respectively.
}
...
With respect to the nonlinear solver, the displacement tolerance of the $\mathcal{\ell}_2$ norm for the Newton update is taken to be $10^{-5}$ whereas the relative and absolute tolerances for the forces is $10^{-8}$. The relative convergence criteria for linear solver is $10^{-6}$.
...
\begin{table}
  ...
  \hline
  \end{tabular}
  \caption{Parameters for the 2D benchmark.
  }
...
\begin{table}
  ...
  \hline
  \end{tabular}
  \caption{Parameters for the 3D benchmark.
  }
...
A node on ``Emmy'' cluster has two Xeon 2660v2 ``Ivy Bridge'' chips (10 cores per chip + SMT) running at 2.2 GHz with 25 MB shared cache per chip and 64 GB of RAM. Intel's compiler version 18.03 with flags ``-O3 -march=native'' and Intel MPI version 18.03 are used.
