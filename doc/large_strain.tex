...
F(\overline{\gz u}, \delta \gz u) + \D_{\Delta \gz u} F(\overline{\gz u}, \delta \gz u) \cdot \Delta \gz u.
\end{align}
For conservative traction boundary conditions, the directional derivative is given by
\begin{equation}
\begin{split}
\D_{\Delta \gz u}  F(\overline{\gz u}, \delta \gz u) \cdot \Delta \gz u
&=
\int_{\mcl B_0} \left[\D_{\Delta \gz u} \gz \tau \cdot \Delta \gz u\right] : \overline{\grad^s} \, \delta \gz u \, \d V
\\
& +
\int_{\mcl B_0} \overline{\gz \tau} : \left[ \Grad \, \delta \gz u \cdot
\left[\D_{\Delta \gz u}  \gz F^{-1} \cdot \Delta \gz u\right]
\right] \d V.
\end{split}
\end{equation}
...
Since each of these $2\times d$ loops has a complexity of $n$ and we perform them for $n^d$ values in quadrature points or values in degrees of freedoms simultaneously, we end up with an arithmetic complexity of $\mathcal{O}(dn^{d+1})$.
...
The force density $12.5 \times 10^3 \rm{N/mm^2}$ or $12.5 \times 10^3 \rm{N/mm^3}$ is applied in 5 steps for the 2D and the 3D problems, respectively.
