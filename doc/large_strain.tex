...
\author[3]{Martin Kronbichler}
\author[1,4]{Paul Steinmann}
...
\address[3]{\orgdiv{Institute for Computational Mechanics}, \orgname{Technical University of Munich}, \orgaddress{\country{Germany}}}

\address[4]{\orgdiv{Glasgow Computational Engineering Center (GCEC)}, \orgname{University of Glasgow}, \orgaddress{\country{United Kingdom}}}
...
more elaborate as compared to the common operators in linear and non-linear fluid mechanics. For matrix-free schemes using fast integration via sum factorization, the main challenge is to find a representation of the linearization point, either in referential or spatial configurations, at the quadrature points that is cheap enough to be evaluated on the fly.
...
For completeness we mention that the fully referential linearization of the here considered constitutive equation \eqref{eq:neo_hookean_energy} results in the following equivalent bilinear form
\begin{align}
  A_{ij} \equiv a(\gz N_i, \gz N_j) =&
  \int_{\mcl B_0}
  \grad \gz N_i : D_{F}P : \grad \gz N_j
  \d V
...
and spatial coordinates also involves a data access via the so-called metric
...
both the Jacobian in the referential and the spatial frame, whereas the other
...
This library offers a number of paradigms by which to parallelize a finite-element code, and as a member of the xSDK (Extreme-scale Scientific Software Development Kit) ecosystem \citep{Bartlett2017b} aiming to support exascale computing.
...
For the linear solver, \texttt{deal.II}'s implementation of the preconditioned conjugate gradient (CG) solver for symmetric positive-definite systems is consistently usedg.
...
as the matrix becomes more and more dense. In order to increase visibility, the figures also report the throughput of the matrix vector product, computed by the ratio of the number of degrees of freedom over the time for a matrix-vector product, in panels (a) and (d).
...
(i) `read / write' denotes reading and writing from/to a global vector (lines 2 and 13) with indirect addressing of gather/scatter type;
...
\todo[inline]{Do we want to update Tables \ref{tab:numbers_2d} and \ref{tab:numbers_3d}?}
...
\todo[inline]{Do we want to update? I can run on up to 8 nodes with CSL. This allows me to do 3 steps of weak scaling in 2D, 1 socket, 2 nodes, 8 nodes, and two steps in 3D, 1 node and 8 nodes. Not very large but enough to show our point?}
