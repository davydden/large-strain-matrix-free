...
using namespace dealii;
  ...
  public:
    ...
    void Tvmult (LinearAlgebra::distributed::Vector<double> &dst,
                 const LinearAlgebra::distributed::Vector<double> &src) const;
    void vmult_add (LinearAlgebra::distributed::Vector<double> &dst,
                    const LinearAlgebra::distributed::Vector<double> &src) const;
    void Tvmult_add (LinearAlgebra::distributed::Vector<double> &dst,
                     const LinearAlgebra::distributed::Vector<double> &src) const;
  ...
  private:
    ...
    */
   void do_operation_on_cell(FEEvaluation<dim,fe_degree,n_q_points_1d,dim,number> &phi_reference,
  ...
  template <int dim, int fe_degree, int n_q_points_1d, typename number>
  void
  NeoHookOperatorAD<dim,fe_degree,n_q_points_1d,number>::precondition_Jacobi(LinearAlgebra::distributed::Vector<number> &dst,
                                            const LinearAlgebra::distributed::Vector<number> &src,
                                            const number omega) const
  {
  ...
  template <int dim, int fe_degree, int n_q_points_1d, typename number>
  void
  NeoHookOperatorAD<dim,fe_degree,n_q_points_1d,number>::vmult (LinearAlgebra::distributed::Vector<double>       &dst,
                                                const LinearAlgebra::distributed::Vector<double> &src) const
  {
    dst = 0;
    vmult_add (dst, src);
  ...
  template <int dim, int fe_degree, int n_q_points_1d, typename number>
  void
  NeoHookOperatorAD<dim,fe_degree,n_q_points_1d,number>::Tvmult (LinearAlgebra::distributed::Vector<double>       &dst,
                                                 const LinearAlgebra::distributed::Vector<double> &src) const
  {
    dst = 0;
    vmult_add (dst,src);
  ...
  template <int dim, int fe_degree, int n_q_points_1d, typename number>
  void
  NeoHookOperatorAD<dim,fe_degree,n_q_points_1d,number>::Tvmult_add (LinearAlgebra::distributed::Vector<double>       &dst,
                                                     const LinearAlgebra::distributed::Vector<double> &src) const
  {
    vmult_add (dst,src);
  }
  ...
  template <int dim, int fe_degree, int n_q_points_1d, typename number>
  void
  NeoHookOperatorAD<dim,fe_degree,n_q_points_1d,number>::vmult_add (LinearAlgebra::distributed::Vector<double>       &dst,
                                                    const LinearAlgebra::distributed::Vector<double> &src) const
  {
    ...
    local_apply_cell(*data_reference, dst, src,
                           ...
                           const std::pair<unsigned int,unsigned int> &cell_range) const
  {
    ...
    FEEvaluation<dim,fe_degree,n_q_points_1d,dim,number> phi_solution (*data_reference);
    FEEvaluation<dim,fe_degree,n_q_points_1d,dim,number> phi_reference(*data_reference);

    Assert (phi_solution.n_q_points == phi_reference.n_q_points, ExcInternalError());
    ...
    for (unsigned int cell=cell_range.first; cell<cell_range.second; ++cell)
      {
        ...
        AlignedVector<VectorizedArray<number>> local_diagonal_vector(phi_reference.dofs_per_component*phi_reference.n_components);
          ...
          for (unsigned int ic=0; ic<phi_reference.n_components; ++ic)
            {
                ...
                for (unsigned int jc=0; jc<phi_reference.n_components; ++jc)
                  {
                    ...
                    phi_reference.begin_dof_values()  [ind_j] = VectorizedArray<number>();
                  }
              ...
              local_diagonal_vector[ind_i] = phi_reference.begin_dof_values()[ind_i];
            }
        ...
        for (unsigned int i=0; i<phi_reference.dofs_per_component; ++i)
          for (unsigned int ic=0; ic<phi_reference.n_components; ++ic)
            {
              const auto ind_i = i+ic*phi_reference.dofs_per_component;
              phi_reference.begin_dof_values()[ind_i] = local_diagonal_vector[ind_i];
            }
    ...
    for (unsigned int q=0; q<phi_reference.n_q_points; ++q)
      {
        ...
        for (unsigned int vec=0; vec<VectorizedArray<number>::n_array_elements; ++vec)
        {
          ...
          for (unsigned int el = 0; el < Tensor<2,dim, ADType>::n_independent_components; ++el)
            grad_u[grad_u.unrolled_to_component_indices(el)] = ADType(Tensor<2,dim, ADType>::n_independent_components,
                                                                      el, tmp_grad[grad_u.unrolled_to_component_indices(el)]);
          ...
          {
              const TableIndices<2> tbl_idx_i = P.unrolled_to_component_indices(idx_i);
              for (unsigned int idx_j =0; idx_j < P.n_independent_components; ++idx_j)
              {
                  const TableIndices<2> tbl_idx_j = P.unrolled_to_component_indices(idx_j);
        ...
        phi_reference.submit_gradient(double_contract<2,0,3,1>(H,phi_reference.get_gradient(q)),q);
