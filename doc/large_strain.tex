...
the limiting factor in classical finite element solvers.
    ...
    We find that matrix-free methods for finite-strain solid mechanics are very promising, outperforming linear matrix-based schemes by two to five times,
...
In order to avoid the repeated calculation of the Kirchhoff stress, Algorithm \ref{alg:mf_tensor2} caches its value for each element and quadrature point.
...
To that end, we cache two scalars that depend on the Jacobian determinant $J$.
...
the implementation that caches the second-order Kirchhoff stress tensor and evaluates terms in the current configuration is faster than
