...
%\newcommand{\changeDD}[1]{\textcolor{red}{#1}}
...
\begin{algorithm}[!ht]
  ...
  \ForEach{ element $K \in \Omega^h$ }{
        \changeDD{extract local vector values on this element $\overline{\gz u^h}_K$, $\gz x_K$ } \;
...
\begin{algorithm}[!ht]
  ...
  \Output{action of the FE tangent operator \eqref{eq:algebraic_tangent} on $\gz x$}
  \ForEach{ element $K \in \Omega^h$ }{
        \changeDD{extract local vector values on this element $\gz x_K$ } \;
        evaluate the following tensors at each quadrature point using sum factorization :\\
        \enskip$\gz g_s := \overline{\grad^s} \gz x_K$ \tcp*{2nd order symmetric}
        \enskip$\gz g := \overline{\grad} \gz x_K$ \tcp*{2nd order}
        \ForEach{quadrature point $q$ on $K$}{
...
For example, \changeDD{the} deformation gradient $\gz F = \gz I + \Grad \, \overline{\gz u^h}$ (second-order tensor) is evaluated at the specific quadrature point of all elements within the ``block''.
