...
In this section we briefly summarize balance equations of fine-strain hyperelastic continuum in Lagrangian (also known as referential) description.
...
\gz \tau : (\Grad \, \delta \gz u : \gz F^{-1}) =
\gz \tau : \grad \, \delta \gz u \, ,
\end{align}
and therefore
\begin{align}
  F(\gz u, \delta \gz u) =
  \int_{\mcl B_0} \gz \tau : \grad \, \delta \gz u \d V \,
  -
  \int_{\partial \mcl B_0^N} \overline{\gz T} \cdot \delta \gz u \d S
  = 0 \, .
\end{align}
...
that is $\gz \tau = \gz F \cdot \gz S \cdot \gz F^T$.
