...
% Kinematics
%
Deformation of the body $\mcl B$ from the reference configuration $\mcl B_0$ to the current configuration $\mcl B_t$ at time $t$
is defined via the mapping $\gz \varphi_t: \mcl B \rightarrow \mathbb E^3$, which places material points of $\mcl B$ in Euclidean space $\mathbb E^3$.
...
The mapping has to be one-to-one and exclude a self penetration, consequently the Jacobian has to be positive $J = \det \gz F > 0$.
Deformation gradient can be split in the proper orthogonal rotation tensor $\gz R$ and the symmetric right $\gz U$ and left $\gz V$ stretch tensors as $\gz F = \gz R \gz U = \gz V \gz R$.
...
% strain measures
%
In what follows we characterize the material behavior using the Green-Lagrange strain tensor $\gz E:=\frac{1}{2}\left[\gz C - \gz I\right]$, where $\gz C$ is right Cauchy-Green tensor $\gz C := \gz F^T \cdot \gz F = \gz U^2$. Clearly $J = \sqrt{\det \gz C}$ and $\partial (\bullet)/\partial \gz E = 2\, \partial (\bullet)/\partial \gz C$. We will also use the left Cauchy-Green tensor $\gz b := \gz F \cdot \gz F^T = \gz V^2$.
...
\end{align}
where $\overline{\gz T}$ is the prescribed loading at the Neumann part of the boundary $\partial \mcl B_0^N$ in reference configuration, $\mcl \psi$ denotes the strain-energy per unit reference volume and $\gz u$ should satisfy the prescribed Dirichlet boundary conditions $\gz u = \overline{\gz u}$ on $\partial \mcl B_0^D := \partial \mcl B_0 \setminus \partial \mcl B_0^N \neq \emptyset $.
...
In order to apply the Newton method, linearization of \eqref{eq:weak_form} around a given solution field $\overline{\gz u}$ is required. This is given by
\begin{align}
...
\end{align}
This can be simplified \cite{Wriggers2008} to
\begin{align}
\D_{\Delta \gz u} F(\overline{\gz u}, \delta \gz u)
  =
...
\label{eq:tangent}
\end{align}
Here $\overline{(\bullet)}$ is used to denote quantities evaluated using the displacement field $\overline {\gz u}$ and the fourth order tangent tensor is push forward of the material tangent $J \boldsymbol{\mathcal{C}} = \chi\left( 4 \frac{d^{2} \psi \left( \mathbf{C} \right)}{d \mathbf{C} \otimes d \mathbf{C}} \right)$. The first term in \eqref{eq:tangent} is called material part of the tangent operator as it is related to the linearization of the Kirchhoff stress. Whereas the second term in \eqref{eq:tangent} is called geometric part of the tangent since it originates from linearization of the deformation gradient $\gz F$.
...
Algorithm \ref{alg:mf_scalar} caches a single scalar which involves logarithm of Jacobian -- a computationally demanding operation compared to additions and multiplications. As evident from Section \ref{sec:theory}, in order to apply the operator we need to re-evaluate Kirchhoff stress at each quadrature point. This in turn requires evaluation of deformation gradient $\gz F$ and therefore gradients with respect to the undeformed configuration.
